...
import { resolveDeepPathDetail } from '@aperas/core/apeironNgn/resolveCreate';
import { wrap } from '@aperas/core/apeironNgn/node';
...
function resolvePlain(store: Store, paths: string[], base?: string) {
  ...
  for (const pathArg of paths) {
    ...
    lines.push(`aperas://id/${id}  [${displayLabel(id, node)}]  ${(node.title as string) ?? ''}`);
  }
  return { lines };
}
...
function resolveCreateHolder(store: Store, pathArg: string, base: string | undefined, titles: string[]) {
  ...
  const lines = detail.trace.map((entry) => `aperas://id/${entry.id}  [${entry.kind}]  ${entry.title}  ${entry.created ? '(created holder)' : '(existing)'}`);
  return { lines };
}
