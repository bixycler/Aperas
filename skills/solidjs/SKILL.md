...
### Reconciling polled/re-fetched data into a store

`<For>` reconciles by **object reference**, not by an id field. A `createResource` re-fetch
(e.g. on a poll interval) returns a brand-new object graph every time — even when the
underlying data is unchanged, every item has a new reference. `<For>` can't tell those
items are "the same", so it tears down and remounts the entire list on every fetch: lost
local state (open editors, focus, scroll position) and a visible flicker.

`reconcile` fixes this by diffing the new data into an existing store by key, preserving
the reference of anything unchanged and patching only what actually changed:

```tsx
import { createStore, reconcile } from "solid-js/store";

const [data, { refetch }] = createResource(source, fetchData);
const [store, setStore] = createStore<Item[]>([]);

createEffect(() => {
  if (data()) setStore(reconcile(data()!, { key: "id" }));
});

// <For each={store}> now only remounts items whose `id` is new or whose
// content actually changed — unrelated items keep their component instance.
<For each={store}>{(item) => <Row item={item} />}</For>
```

Without this, a resource driving a `<For>` under any polling/auto-refresh scheme will
remount the whole list on every tick, no matter how small the actual change was.
...
6. **Feeding `<For>` straight from a polled/re-fetched `createResource`** — every fetch is a
   fresh object graph with new references, so `<For>` remounts the entire list each time.
   Reconcile into a store first (see *Reconciling polled/re-fetched data into a store* above).
