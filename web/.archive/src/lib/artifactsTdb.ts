/**
 * Aperas Phase 0: Artifact Tracking & On-Demand Ingestion — TerminusDB-backed half.
 *
 * Split out of `artifacts.ts` (Aperas-apeironngn-design.md §4 rollout, archiving step) so that
 * file could hold only the engine-agnostic pieces `apeironNgn/artifacts.ts`/`apeironNgn/node.ts`/
 * `apeironNgn/folders.ts` still depend on (`getArtifactsDir`, `listArtifactFiles`,
 * `computeFileHash`, `isReadmeFilename`, `extractLinkCodes`) — everything here talks to a live
 * TerminusDB `client` and moves to `.archive/` alongside `kgCli.ts`/`client.ts`/`crud.ts`/
 * `graphql.ts`, which are this file's only callers.
 *
...
import { parseMarkdownTree, extractAbstract, stampParents, WIKILINK_PREDICATE } from './astParser';
...
import { resolveNodeRefOrNull, resolveIdToPath } from './nodeRefTdb';
import { updateBlockNode } from './crudTdb';
import { getArtifactsDir, listArtifactFiles, computeFileHash, countBlocks, extractLinkCodes, type PendingLinkCodes } from './artifacts';
