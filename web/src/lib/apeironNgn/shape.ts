...
/** Aperas-treeview-design.md §3/§7/§11 — lightweight identity: still no auth, no `BASE_NODE_FIELDS`
 *  (not a `TreeNode`), but now addressable (`handle`, a stable, human-chosen slug) with a display
 *  `name`, an open unenforced `kind` label (suggested `"human"`/`"agent"` — `Optional`, so "not yet
 *  known" is just left unset, no sentinel value needed), and a `preferences` bag reusing
 *  `Link.props`' own `Set<StringProp>` embed pattern rather than a bespoke shape. Named `handle`,
 *  not `id` — every node already has an opaque intrinsic id
 *  (`ApeironInstance.id`, a real class property, not shape-driven); a schema field literally named
 *  `id` would shadow/collide with it. No `views` field — a `Profile`'s owned `TreeView`s are found
 *  via `backlinks(store, id, 'profile')`, same as every other one-to-many relationship here (§3). */
export const PROFILE_SHAPE: ClassShape = {
  handle: { cardinality: 'one' },
  name: { cardinality: 'optional' },
  kind: { cardinality: 'optional' },
  preferences: { cardinality: 'set', storageKind: 'embed' },
};

/** Aperas-treeview-design.md §3-§6/§11 — an i-view: a lens over the one real `TreeNode`/`Link`
 *  graph, not a parallel structure. `unfolds` replaces `BASE_NODE_SHAPE.unfolded` — fold state
 *  lives here, per view, instead of as a single flag shared by every viewer. Points at either a
 *  `TreeNode` (`BlockNode`/`ArtifactNode`/`FolderNode`) or a `Link` — both are plain bare-id
 *  references here, never minted fresh by this field (an `unfolds` write always names something
 *  that already exists), so `storageKind: 'reference'` is correct even for the `Link` case, not
 *  `'embed'`. `name` tightened from `Optional` to `One` (§11) — every `TreeView` is now always
 *  created with one, via `kg:profile create-view` or the `"default"` bootstrap, playing exactly
 *  the same "stable, globally-unique addressable handle" role `Profile.handle` does. */
