 ...
 * artifact/folder path, extended to a full deep path — Aperas-deep-path-resolution-design.md).
 * Used by `kgCli.ts` for CLI addressing, and by `artifacts.ts`'s wikilink extraction.
 *
 * Circular-import note: this module imports `getArtifactRecord`/`getFolderRecord` from
 * artifacts.ts/folders.ts, and artifacts.ts now imports `resolveNodeRefOrNull`/`resolveIdToPath`
 * back from here (for wikilink resolution) — a real cycle, unlike directResolve.ts's deliberate
 * leaf-module design. Safe here specifically because every export on both sides is a plain
 * `function` declaration (hoisted, fully defined before either module's body finishes
 * evaluating) and every cross-module call happens inside an async function body at runtime, long
 * after both modules have finished loading — never at module-top-level. Confirmed live via
 * `kg:ingest` after this was wired up.
 */
